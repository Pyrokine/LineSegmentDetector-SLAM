...
Mat GaussImageBK = Mat::zeros(128, 413, CV_64FC1);
#endif
...
void drawText2(string text, int x, int y);
void drawDeg(Mat degMap, Mat curMap, Mat banMap, int x, int y, int cenDegX, int cenDegY, int regDegX, int regDegY);
...
//end-test
	...
	GaussImage.copyTo(GaussImageBK);
...
#ifdef _disp_
				if (GaussImage.ptr<double>(y)[x] > 0)
					drawPoint(mapDispBase, x, y, 140, 180, 210);
#endif // _disp_
			}
...
structRegionGrower RegionGrower(int x, int y, Mat banMap, double regDeg, Mat degMap, double degThre) {
	...
	drawPoint(mapDispBK, x, y, 34, 139, 34);//深绿
	drawPoint(mapDispBase, x, y, 0, 140, 255);//橙色
	int regDegX = x, regDegY = y;
#endif
	...
	int cenDegX = -10, cenDegY = -10;
	for (i = 0; i < growNum; i++) {
		//检验8邻域像素是否满足角弧度阈值
		int m, n;
		int roi_x = regPts_now[0].x, roi_y = regPts_now[0].y;
#ifdef _disp_
		if (abs(roi_x - cenDegX) >= 17 || abs(roi_y - cenDegY) >= 7) {
			cenDegX = roi_x;
			cenDegY = roi_y;
		}
#endif
				...
				if (m >= 0 && n >= 0 && m < yLim && n < xLim) {
					...
					if (curMap.ptr<uint8_t>(m)[n] != 1 && banMap.ptr<uint8_t>(m)[n] != 1){
						...
						if (degDif < degThre) {
							...
							regDegX = n;
							regDegY = m;
							drawPoint(mapDispBK, n, m, 34, 139, 34);//深绿
							drawPoint(mapDispBase, n, m, 0, 140, 255);//橙色
...
void drawText2(string text, int x, int y) {
	int baseLine;
	Size text_size = getTextSize(text, FONT_HERSHEY_SCRIPT_SIMPLEX, 1, 1, &baseLine);
	putText(mapDisp, text, Point(x, y), FONT_HERSHEY_SIMPLEX, 0.8, Scalar(1, 1, 1), 1, 8);
}
...
void drawDeg(Mat degMap, Mat curMap, Mat banMap, int x, int y, int cenDegX, int cenDegY, int regDegX, int regDegY) {
	...
	cenDegY += offsetY - 7;
	int i, j;
	for (i = cenDegY; i <= cenDegY + 14; i++) {
		for (j = cenDegX; j <= cenDegX + 34; j++) {
			double degree = degMap.ptr<double>(i)[j];
			if (j == x && i == y && (curMap.ptr<uint8_t>(i)[j] == 1 || GaussImageBK.ptr<double>(i)[j] < 0.000001 || banMap.ptr<uint8_t>(i)[j] != 0))
				rectangle(mapDisp, Point((j - cenDegX) * 15, 384 + (i - cenDegY) * 15), \
					Point((j - cenDegX) * 15 + 14, 384 + (i - cenDegY) * 15 + 14), Scalar(0, 0, 1), FILLED);//红色
			else if (j == x && i == y && curMap.ptr<uint8_t>(i)[j] == 0)
				rectangle(mapDisp, Point((j - cenDegX) * 15, 384 + (i - cenDegY) * 15), \
					Point((j - cenDegX) * 15 + 14, 384 + (i - cenDegY) * 15 + 14), Scalar(0, 252 / 255.0, 124 / 255.0), FILLED);//绿色
			else if (j == regDegX && i == regDegY)
				rectangle(mapDisp, Point((j - cenDegX) * 15, 384 + (i - cenDegY) * 15), \
					Point((j - cenDegX) * 15 + 14, 384 + (i - cenDegY) * 15 + 14), Scalar(240 / 255.0, 230 / 255.0, 140 / 255.0), FILLED);//卡其布
			else if (curMap.ptr<uint8_t>(i)[j] == 1)
				rectangle(mapDisp, Point((j - cenDegX) * 15, 384 + (i - cenDegY) * 15), \
					Point((j - cenDegX) * 15 + 14, 384 + (i - cenDegY) * 15 + 14), Scalar(34 / 255.0, 139 / 255.0, 34 / 255.0), FILLED);//深绿
			else if (GaussImageBK.ptr<double>(i)[j] < 0.000001)
				rectangle(mapDisp, Point((j - cenDegX) * 15, 384 + (i - cenDegY) * 15), Point((j - cenDegX) * 15 + 14, 384 + (i - cenDegY) * 15 + 14), \
					Scalar(155 / 255.0, 211 / 255.0, 254 / 255.0), FILLED);//橙色
			else if (banMap.ptr<uint8_t>(i)[j] != 0)
				rectangle(mapDisp, Point((j - cenDegX) * 15, 384 + (i - cenDegY) * 15), Point((j - cenDegX) * 15 + 14, 384 + (i - cenDegY) * 15 + 14), \
					Scalar(140 / 255.0, 180 / 255.0, 210 / 255.0), FILLED);//晒黑
			else
				rectangle(mapDisp, Point((j - cenDegX) * 15, 384 + (i - cenDegY) * 15), \
					Point((j - cenDegX) * 15 + 14, 384 + (i - cenDegY) * 15 + 14), Scalar(1, 1, 1), FILLED);//白色
			int loc_x = round(cos(degree) * 5);
			int loc_y = round(sin(degree) * 5);
			int loc_x1 = 7 + (j - cenDegX) * 15 - loc_x;
			int loc_x2 = 7 + (j - cenDegX) * 15 + loc_x;
			int loc_y1 = 384 + 7 + (i - cenDegY) * 15 - loc_y;
			int loc_y2 = 384 + 7 + (i - cenDegY) * 15 + loc_y;
			line(mapDisp, Point(loc_x1, loc_y1), Point(loc_x2, loc_y2), Scalar(0, 0, 0));
			circle(mapDisp, Point(loc_x1, loc_y1), 1, Scalar(0, 0, 0));
		}
	}
}
#endif
